export const EasingFunctions = {
  ...
  easeInElastic: (t) => {
    if (t === 0 || t === 1) return t;
    const c4 = (2 * Math.PI) / 0.3; // tweak the 0.3 to adjust the spring period
  ...
  easeInOutElastic: (t) => {
    if (t === 0 || t === 1) return t;
    const c5 = (2 * Math.PI) / 0.45; // tweak the 0.45 to adjust the spring period
    if (t < 0.5) {
      return -0.5 * Math.pow(2, 20 * t - 10) * Math.sin((20 * t - 11.125) * c5);
    } else {
      return Math.pow(2, -20 * t + 10) * Math.sin((20 * t - 11.125) * c5) * 0.5 + 1;
    }
  },
  easeOutBounce: (t) => {
    const n1 = 7.5625;
